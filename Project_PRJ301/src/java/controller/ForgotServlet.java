...
public class ForgotServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        DAO dao = new DAO();
        Account a = dao.forgotAccount(user, email);
        if (a != null) {
            LocalDateTime currentDateTime = LocalDateTime.now();
            Email handleEmail = new Email();
            String sub = handleEmail.forgotAccount(user);
            String msg = handleEmail.messagePass(currentDateTime, user, a.getPass());
            handleEmail.sendEmail(sub, msg, email);
            request.setAttribute("mess", "Mật khẩu của bạn đã được cấp vui lòng kiểm tra email của bạn.");
            request.getRequestDispatcher("forgotpass.jsp").forward(request, response);
        } else {
